...
delay_1ms:
    ...
    MOVWF TMR0L, A
    BCF INTCON, 2, A ; Clear ((INTCON) and 0FFh), 2, a flag before starting
    MOVLW 0b10000010 ; Timer ON, 16-bit, internal clock, 1:8 prescaler
    MOVWF T0CON, A

wait_1ms:
    BTFSS INTCON, 2, A ; Check ((INTCON) and 0FFh), 2, a flag
    GOTO wait_1ms
    BCF INTCON, 2, A ; Clear flag
    BCF T0CON, 7, A ; Turn off timer (((T0CON) and 0FFh), 7, a = 0)
    RETURN
