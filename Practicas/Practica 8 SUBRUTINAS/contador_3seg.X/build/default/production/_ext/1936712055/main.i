# 1 "../Contador_1ms.X/main.s"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 296 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "../Contador_1ms.X/main.s" 2
# 1 "../Contador_1ms.X/configbits.s" 1
...
delay_1sec:
    ; Disable Timer0
    BCF T0CON, 7 ; ((T0CON) and 0FFh), 7, a bit (bit 7)
    MOVLW 0b10000101
    MOVWF T0CON
    ...
    MOVWF TMR0L
    ; Clear overflow flag
    BCF INTCON, 2 ; ((INTCON) and 0FFh), 2, a bit (bit 2)
    ; Start Timer0
    BSF T0CON, 7 ; ((T0CON) and 0FFh), 7, a bit (bit 7)

delay_wait:
    ; Wait for overflow flag
    BTFSS INTCON, 2 ; ((INTCON) and 0FFh), 2, a bit (bit 2)
    GOTO delay_wait
