...
void mostrar_mensaje_inicial(void) {
    lcd_clear();
    //lcd_printCenter("", 0);
    lcd_scrollText("Proteccion por contrasena", 0, 100);
    __delay_ms(500);
}
...
void solicitar_password(void) {
    ...
    if (verificar_password(password_ingresado)) {
        ...
        // Mensaje de salida
        lcd_clear();
        lcd_printCenter("Eso es todo", 0);
        lcd_printCenter("No hay mas", 1);
        __delay_ms(4000);
